...
class R2D2Net(torch.jit.ScriptModule):
    __constants__ = ["hid_dim", "out_dim", "num_fflayer", "rnn_type", "num_rnn_layer", "hand_size"]

    def __init__(
        self, device, in_dim, hid_dim, out_dim, num_fflayer, rnn_type, num_rnn_layer, hand_size
    ):
        super().__init__()
        self.in_dim = in_dim
        ...
        print("self rnn type is ", self.rnn_type)
        ...
        if self.rnn_type == 'lstm':
            print("rnn type is lstm, creating cell")
            self.rnn = nn.LSTM(
                self.hid_dim,
                self.hid_dim,
    ...
    def act(
        ...
        x = self.net(priv_s)
        print("rnn type inside act is ", self.rnn_type)
        if self.rnn_type == 'lstm':
            o, (h, c) = self.rnn(x, (hid["h0"], hid["c0"]))
        elif self.rnn_type == 'gru':
            o, (h) = self.rnn(x, (hid["h0"]))
        a = self.fc_a(o)
        a = a.squeeze(0)
        if self.rnn_type == 'lstm':
            return a, {"h0": h, "c0": c}
    ...
    def forward(
        ...
        if self.rnn_type == 'lstm':
            print("inside lstm within forward...")
            if len(hid) == 0:
                o, (h, c) = self.rnn(x)
            else:
                o, (h, c) = self.rnn(x, (hid["h0"], hid["c0"]))
    ...
    def cross_entropy(self, net, rnn_o, target_p, hand_slot_mask, seq_len):
        # target_p: [seq_len, batch, num_player, 5, 3]
        # hand_slot_mask: [seq_len, batch, num_player, 5]
        logit = net(rnn_o).view(target_p.size())
    ...
    def pred_loss_1st(self, rnn_o, target, hand_slot_mask, seq_len):
        return self.cross_entropy(self.pred, rnn_o, target, hand_slot_mask, seq_len)
...
class R2D2Agent(torch.jit.ScriptModule):
    ...
    def __init__(
        ...
        self.online_net = R2D2Net(
            device, in_dim, hid_dim, out_dim, num_fflayer, rnn_type, num_rnn_layer, hand_size
        ).to(device)
    ...
    def td_error(self, obs, hid, action, reward, terminal, bootstrap, seq_len, stat):
        ...
        online_qa, greedy_a, _, rnn_o = self.online_net(
        ...
        if self.vdn:
            ...
            rnn_o = rnn_o.view(max_seq_len, bsize, num_player, -1)
    ...
    def aux_task_iql(self, rnn_o, hand, seq_len, rl_loss_size, stat):
        ...
        pred_loss1, avg_xent1, _, _ = self.online_net.pred_loss_1st(
            rnn_o, own_hand, own_hand_slot_mask, seq_len
        )
    ...
    def aux_task_vdn(self, rnn_o, hand, seq_len, rl_loss_size, stat):
