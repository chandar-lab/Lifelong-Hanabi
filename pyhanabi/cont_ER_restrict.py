    ...
    for task_idx, fixed_agent in enumerate(fixed_agents):
        ...
        for epoch in range(args.num_epoch):
            ...
            for batch_idx in range(args.epoch_len):
                total_task_steps = utils.get_num_acts(act_group.actors) // args.num_player
                print("total task steps after ", batch_idx, " epoch len is ", total_task_steps)
            ...
            if (epoch+1) % args.eval_freq == 0:
                tmp = utils.get_num_acts(act_group.actors) // args.num_player
                print("tmp acts before calling context pause is ", tmp)
                context.pause()
                for eval_fixed_ag_idx, eval_fixed_agent in enumerate(fixed_agents + [fixed_learnable_agent]):
                    ...
                    if args.eval_method == 'few_shot':
                        ...
                        tmp = utils.get_num_acts(act_group.actors) // args.num_player
                        print("tmp acts during finetuning with ", eval_fixed_ag_idx, " is ", tmp)
                        eval_done = False
                        for eval_epoch in range(args.eval_num_epoch):
                            ...
                            for eval_batch_idx in range(args.eval_epoch_len):
                                total_eval_steps = utils.get_num_acts(eval_act_group.actors) // args.num_player
                                print("total eval steps after ", eval_batch_idx, " epoch len is ", total_eval_steps)
