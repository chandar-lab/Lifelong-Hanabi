...
def evaluate_legacy_model(
    ...
    for i, weight_file in enumerate(weight_files):
        ...
        learnable_agent_name = (
            "/".join(weight_files[1].split("/")[:-1])
            + "/"
            + learnable_agent_args["load_learnable_model"].split("/")[-1]
        )
...
if __name__ == "__main__":
    ...
    if learnable_agent_args["load_learnable_model"] != "":
        move_model_0 = (
            "cp "
            + learnable_agent_args["load_learnable_model"]
            + " "
            + final_models_dir
            + "/"
            + "model_epoch0_zero_shot.pthw"
        )
    ...
    for ag1_idx, ag1 in enumerate(weight_1):
        ...
        if ag1_name == "shot.pthw":
            ...
            for fixed_agent_idx in range(len(args.weight_2)):
                ...
                mean_score, sem, perfect_rate = evaluate_legacy_model(
                    weight_files, 1000, 1, 0, learnable_agent_args, args, num_run=5
                )
        ...
        if (
            act_epoch_cnt == learnable_agent_args["num_epoch"] * (cur_task + 1)
