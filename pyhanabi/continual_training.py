    ...
    for task_idx, fixed_agent in enumerate(fixed_agents):
        ...
        for epoch in range(args.num_epoch):
            ...
            for batch_idx in range(args.epoch_len): 
                ...
                if args.ll_algo == "ER":
                    ## here...
                    batch, weight, episodic_memory = utils.make_batch_ER(args, episodic_memory, batch, weight, stat, learnable_agent)
                    stopwatch.time("sample data")

                elif args.ll_algo == "AGEM":
                    b, w, episodic_memory = utils.make_batch_AGEM(args, episodic_memory, stat, learnable_agent)
                    stopwatch.time("sample data")
        ...
        if args.store_sliced_rb:
            if args.episodic_replay_buffer_size < replay_buffer.size():
                replay_buffer.slice(replay_buffer.size() - args.episodic_replay_buffer_size)
            else:
                print("Episodic replay buffer size can't be larger than replay buffer size!!!")
