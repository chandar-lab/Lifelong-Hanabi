...
def parse_args():
    ...
    parser.add_argument("--num_gradient_update", type=int, default=1, help="meta gradient updates")
    ...
    parser.add_argument("--max_train_steps", type=int, default=2000000000)
    parser.add_argument("--max_eval_steps", type=int, default=500000)
    ...
    parser.add_argument("--ft_replay_buffer_size", type=int, default=10000)
    parser.add_argument(
...
if __name__ == "__main__":
    ...
    for task_idx, fixed_agent in enumerate(fixed_agents):
        ...
        context, threads = create_threads(
            args.num_thread, args.num_game_per_thread, act_group_list[task_idx].actors, games,
        )
        context_list.append(context)
        act_group_list[task_idx].start()
        context_list[task_idx].start()
        while replay_buffer.size() < (task_idx + 1) * (args.burn_in_frames // len(fixed_agents)):
        ...
        fine_tune_replay_buffer_list.append(rela.RNNPrioritizedReplay(
            ...
            args.seed + task_idx + 1,
            args.priority_exponent,
            args.priority_weight,
            args.prefetch,
        )
        )
        fine_tune_games = create_envs(
        ...
        fine_tune_act_group = ActGroup(
            args.method,
            args.act_device,
            [fine_tune_learnable_agent, fixed_agent],
            ...
            args.num_player,
            args.is_rand,
            fine_tune_replay_buffer_list[task_idx]
        )
        fine_tune_act_group_list.append(fine_tune_act_group)

        fine_tune_context, fine_tune_threads = create_threads(
            args.eval_num_thread, args.eval_num_game_per_thread, fine_tune_act_group.actors, fine_tune_games,
        )
        fine_tune_context_list.append(fine_tune_context)
        fine_tune_act_group_list[task_idx].start()
        fine_tune_context_list[task_idx].start()
        while fine_tune_replay_buffer_list[task_idx].size() < args.batchsize:
            print("warming up replay buffer:", fine_tune_replay_buffer_list[task_idx].size())
            time.sleep(1)
        fine_tune_context_list[task_idx].pause()
    ...
    tachometers = [utils.Tachometer() for _ in range(len(fixed_agents))]
    ...
    for epoch in range(args.num_epoch):
        ...
        for batch_idx in range(args.epoch_len):
            ...
            for ac in act_group_list:
                ...
                mtl_steps = utils.get_num_acts(learnable_agent_actors)
            ...
            for fine_tune_fixed_ag_idx, fine_tune_fixed_agent in enumerate(fixed_agents):
                ...
                fine_tune_stat.reset()
                fine_tune_tachometer.start()
                # eval_epoch_len is equal to num of gradients
                for fine_tune_batch_idx in range(args.num_gradient_update):
                    fine_tune_num_update = fine_tune_batch_idx + 0 * args.eval_epoch_len
                    # print("fine_tune num updates: ", fine_tune_num_update)
                    if fine_tune_num_update % 1 == 0:
                        fine_tune_learnable_agent_list[fine_tune_fixed_ag_idx].sync_target_with_online()
                    if fine_tune_num_update % 1 == 0:
                        fine_tune_act_group_list[fine_tune_fixed_ag_idx].update_model(
                            fine_tune_learnable_agent_list[fine_tune_fixed_ag_idx])

                    batch, weight = fine_tune_replay_buffer_list[fine_tune_fixed_ag_idx].sample(args.batchsize,
                                                                                                args.train_device)

                    torch.cuda.synchronize()

                    loss, priority = fine_tune_learnable_agent_list[fine_tune_fixed_ag_idx].loss(batch,
                                                                                                 args.pred_weight,
                                                                                                 fine_tune_stat)
                    ...
                    fine_tune_replay_buffer_list[fine_tune_fixed_ag_idx].update_priority(priority[:args.batchsize])

                    fine_tune_stat["loss"].feed(loss.detach().item())
                    fine_tune_stat["grad_norm"].feed(g_norm.detach().item())

                count_factor = args.num_player if args.method == "vdn" else 1
                fine_tune_learnable_agent_actors = [x[0] for x in
                                                    fine_tune_act_group_list[fine_tune_fixed_ag_idx].actors]
                #fine_tune_tachometer.lap(fine_tune_learnable_agent_actors,
                #                         fine_tune_replay_buffer_list[fine_tune_fixed_ag_idx],
                #                         args.eval_epoch_len * args.batchsize, count_factor)
                # fine_tune_stat.summary(0)
                fine_tune_context_list[fine_tune_fixed_ag_idx].pause()
            ...
            for task_idx, fixed_agent in enumerate(fixed_agents):
                ...
                if num_update % args.actor_sync_freq == 0:
                    act_group_list[task_idx].update_model(fine_tune_learnable_agent_list[task_idx])
        ...
        if (epoch + 1) % args.eval_freq == 0:
            for eval_fixed_ag_idx, eval_fixed_agent in enumerate(fixed_agents + [fixed_learnable_agent]):
                ...
                if args.eval_method == 'few_shot':
                    ...
                    if args.optim_name == "Adam":
                        eval_optim = torch.optim.Adam(few_shot_learnable_agent.online_net.parameters(),
                                                      lr=args.final_lr,
                                                      eps=args.eps)
                    elif args.optim_name == "SGD":
                    ...
                    for eval_epoch in range(args.eval_num_epoch):
                        ...
                        for eval_batch_idx in range(args.eval_epoch_len):
                            # print("eval batch idx is ", eval_batch_idx)
                        ...
                        eval_learnable_agent_actors = [x[0] for x in eval_act_group.actors]
                        eval_tachometer.lap(eval_learnable_agent_actors, eval_replay_buffer,
                                            args.eval_epoch_len * args.batchsize, count_factor)
                    ...
                    fs_force_save_name = "model_epoch%d_few_shot_%d" % (
                        total_epochs * len(fixed_agents), eval_fixed_ag_idx)
                    few_shot_model_saved = saver.save(None, few_shot_learnable_agent.online_net.state_dict(),
                                                      force_save_name=fs_force_save_name)
                    print("few shot model saved: %s " % (few_shot_model_saved))

            ## zero shot learnable agent.
            zs_force_save_name = "model_epoch%d_zero_shot" % (total_epochs * len(fixed_agents))
            zero_shot_model_saved = saver.save(None, learnable_agent.online_net.state_dict(),
