...
def evaluate_legacy_model(
    weight_files, num_game, seed, bomb, learnable_agent_args, cont_train_args, num_run=1, verbose=True
):
...
if __name__ == "__main__":
    ...
    if 'load_learnable_model' in learnable_agent_args:  
        ...
        lr_str = learnable_agent_args['method'] 
        if float(learnable_agent_args['shuffle_color']) == 1.0:
            lr_str += "_OP"
        elif float(learnable_agent_args['pred_weight']) > 0:
            lr_str += "_AUX"
        lr_str += "_no_CL"
